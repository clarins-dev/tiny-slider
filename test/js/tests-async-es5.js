...
canFireKeydown = body.getAttribute('data-fire-keyevent') === 'true' ? true : false; // ### base
...
async function testBase() {
  ...
  runTest('Nav items: data-nav, hidden', function () {
    ...
    return navVisible.getAttribute('data-nav') === '0' && navVisible.style.display !== 'none' && navHidden.getAttribute('data-nav') === '3' && navHidden.style.display === 'none';
  });
  var controlsClick = addTest('Controls: click functions'),
      ...
      testGoto = addTest('Goto: Random numbers'); // controls click

  await checkControlsClick(controlsClick, id, 11); // nav click
  ...
  updateTest(navClick, assertion); // keydown events

  if (canFireKeydown) {
    // controls keydown
    await repeat(function () {
      // fire keydown events on left arrow
      fire(info.controlsContainer, 'keydown', {
        'keyCode': 37
      });
    }, 3);
    var prev = info.index,
    ...
    updateTest(controlsKeydown, assertion); // nav keydown

    var navContainer = info.navContainer,
        wrapperLeft = innerWrapper.getBoundingClientRect().left; // focus on the 1st nav item

    navItems[0].focus(); // fire keydown event on right arrow
    // the 2nd nav item get focused

    fire(navContainer, 'keydown', {
      'keyCode': 39
    });
    assertion = document.activeElement === navItems[1]; // press "Enter"
    ...
    if (assertion) {
      assertion = getAbsIndex(current, 0, info) === 1 && navItems[1].className.indexOf(navActiveClass) >= 0 && compare2Nums(currentSlide.getBoundingClientRect().left, wrapperLeft);
...
async function testStartIndex() {
  ...
  function startIndexFn(id) {
    var info = sliders[id].getInfo(),
        ...
        items = info.items,
        slideCount = info.slideCount,
        cloneCount = info.cloneCount; // console.log(slideItems[getStartIndex(cloneCount, slideCount, items, options[id])].getBoundingClientRect().left);
...
async function testNonLoop() {
  ...
  var assertion,
      ...
      current;
  nextButton.click();
  assertion = !prevButton.hasAttribute('disabled'); // click next button (slideCount - items) times
...
function center_regular(id) {
  var slider = sliders[id],
      nextButton = slider.getInfo().nextButton;
  addTitle(id);
...
function testResponsive1() {
  ...
  async function responsive1Tests() {
    try {
      ...
      assertionEdgePadding = compare2Nums(wrapperRect.left, edgePadding) && compare2Nums(wrapperRect.right, Number(bps[0]) + 20 - (edgePadding - gutter)); // resize window
...
function testResponsive2() {
  ...
  async function responsive2Tests() {
    try {
      ...
      assertionAutoplay = getComputedStyle(autoplayButton, null).display === 'none' && autoplayButton.getAttribute('data-action') === 'start' && firstRect.left === slideItems[index].getBoundingClientRect().left; // console.log(assertionControls, assertionNav, assertionAutoplay);
...
function testResponsive3() {
  ...
  async function responsive3Tests() {
    try {
      ...
      assertionAutoplay = str.substring(str.length - 4) === 'stop'; // resize window
...
function testResponsive4() {
  ...
  updateTest(testArrowKeysT, '-notsure'); // var newWindow = document.createElement('iframe');
  // newWindow.setAttribute('frameBorder', '0');
  // newWindow.style.cssText = 'width: ' + (Number(bps[1]) + 20) + 'px; height: 1000px; border-width: 0; overflow: hidden;';
  // newWindow.src = id + prefix + '.html';
  // if (newWindow.addEventListener) {
  ...
  function responsive4Tests() {
    if (canFireKeydown) {
      ...
      new Promise(function (resolve) {
        left = container.getBoundingClientRect().left; // fire keydown event on right arrow
        ...
        return new Promise(function (resolve) {
          assertionArrowKeys = container.getBoundingClientRect().left !== left; // console.log(assertionArrowKeys);
...
function testResponsive5() {
  ...
  async function responsive5Tests() {
    try {
      ...
      await wait(1000);
      assertionFixedWidth = first.clientWidth === getFW(windowWidth) && compare2Nums(wrapper.getBoundingClientRect().left, first.getBoundingClientRect().left); // console.log(first.clientWidth + '|' + getFW(windowWidth) + '|' + wrapper.getBoundingClientRect().left + '|' + first.getBoundingClientRect().left);

      assertionAutoHeight = wrapper.style.height === ''; // if (!assertionFixedWidth) {
      ...
      if (assertionFixedWidth) {
        assertionFixedWidth = first.clientWidth === getFW(windowWidth) + 100 && compare2Nums(wrapper.getBoundingClientRect().left, first.getBoundingClientRect().left); // console.log(first.clientWidth + ' | ' + (getFW(windowWidth) + 100) + ' | ' + wrapper.getBoundingClientRect().left + ' | ' + first.getBoundingClientRect().left);
        // if (!assertionFixedWidth) {
      ...
      if (assertionAutoHeight) {
        assertionAutoHeight = wrapper.style.height === first.clientHeight + 'px'; // if (!assertionAutoHeight) {
...
function testResponsive6() {
  ...
  async function responsive6Tests() {
    try {
      ...
      assertionEdgePadding = left === edgepadding && right === viewport - (edgepadding - gutter); // if (!assertionEdgePadding) {
      ...
      if (assertionEdgePadding) {
        ...
        assertionEdgePadding = left === 0; // if (!assertionEdgePadding) {
      ...
      assertionControlsNav = controlsDisplay === 'none' && navDisplay === 'none'; // if (!assertionControlsNav) {
...
function testGutter() {
  ...
  runTest('Slide: gutter', function () {
    var slideItems = info.slideItems,
        cloneCount = info.cloneCount,
        firstRect = slideItems[cloneCount].getBoundingClientRect(),
        secondRect = slideItems[cloneCount + 1].getBoundingClientRect(); // There is no "gap" between the two slides
...
function testArrowKeys() {
  var id = 'arrowKeys',
      ...
      info = slider.getInfo();
  addTitle(id);
  var test = addTest('Slides: keydown'); // can fire keydown event

  if (canFireKeydown) {
    var assertion,
        container = info.container,
        slideBy = info.slideBy,
        index = slider.getInfo().index; // fire keydown event on right arrow
    ...
    updateTest(test, assertion); // can't fire keydown event
    // manual test needed
  } else {
    updateTest(test, '-notsure');
  }
...
async function testAutoplay() {
  ...
  var testClick = addTest('autoplayButton: click'),
      ...
      assertion; // click autoplay button once => pause
  ...
  updateTest(testClick, assertion); // test autoplay

  await testAutoplayFn(id, test1, timeout, false); // test autoplay pause
...
async function testAnimation2() {
  ...
  var test = addTest('Slides: position after click'),
      ...
      assertion; // click next button *count* times
...
function checkSlidesAttrs(id) {
  ...
  return slideItems.length === slideCount + cloneCount * mul && containsClasses(firstVisible, ['tns-item']) && firstVisible.id === id + '-item' + 0 && !firstVisible.hasAttribute('aria-hidden') && !firstVisible.hasAttribute('tabindex') && firstVisiblePrev.id === '' && firstVisiblePrev.getAttribute('aria-hidden') === 'true' && firstVisiblePrev.getAttribute('tabindex') === '-1' && lastVisible.id === id + '-item' + (items - 1) && !lastVisible.hasAttribute('aria-hidden') && !lastVisible.hasAttribute('tabindex') && lastVisibleNext.getAttribute('aria-hidden') === 'true' && lastVisibleNext.getAttribute('tabindex') === '-1' && compare2Nums(firstVisible.clientWidth, windowWidth / items) && checkLastItem;
}
...
async function checkControlsClick(test, id, count, vertical, islast) {
  ...
  function getAssertion(absIndex) {
    var index = sliders[id].getInfo().index,
        ...
        checkLastEdge = options[id]['fixedWidth'] ? true : compare2Nums(last.getBoundingClientRect()[edge2], wrapper.getBoundingClientRect()[edge2]); // if (id === 'customize') {
    // console.log(absIndex, index%slideCount);
    // }

    return !first.hasAttribute('aria-hidden') && !first.hasAttribute(tabindex) && !last.hasAttribute('aria-hidden') && !last.hasAttribute(tabindex) && compare2Nums(first.getBoundingClientRect()[edge1], wrapper.getBoundingClientRect()[edge1]) && checkLastEdge;
